//
//  Plotter sounds - joydevisualisation
//
//
  ...
  if (frameSkipper === 0) {
    ...
    if (startedCapture) {
      ...
      for (let i = 2; i < dataArray.length / 2; i++) {
        const thisVal = ((h / 10) * (dataArray[i] / 160))
      ...
      const firstQuarterHeights = heights.slice(0, heights.length / 2)
      const reversedFirstQuarterHeights = firstQuarterHeights.reverse()
      heights = [...reversedFirstQuarterHeights, ...heights]

      if (heights.length) {
        // Now we need to modify the value by multiplying it by a sin wave
        // where the length of the sinwave is the length of the heights array
        const sinWave = heights.map((h, i) => h * ((Math.sin(i / heights.length * 3) * 2) / 2) + 0.5)
        joyLines.push(sinWave)
      }
      ...
      for (let c = 0; c < joyLines.length; c++) {
        ...
        const joyHeight = h - (border * 3)
        const joyStep = joyHeight / maxLines
        const linePosition = h - border - (joyStep * (joyLines.length - c))
        const line = []
        // Move to the first point
        const startX = border
        const startY = linePosition - joyLines[c][0]
        // console.log(startX)
        line.push({
          x: startX / w,
          y: startY / h
        ...
        for (let i = 1; i < joyLines[c].length; i++) {
        ...
        // We are going to fill the shape in
        ctx.beginPath()
        ...
        // Now draw down to the bottom of the canvas
        ctx.lineTo(line[line.length - 1].x * w, h)
        ctx.lineTo(line[0].x * w, h)
        ctx.lineTo(line[0].x * w, line[0].y * h)
        // Stroke the path
        ctx.fill()
...
<!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" 
  ...
  for (let i = 0; i < allLines.length; i++) {
    ...
    output += '<path style="fill:#FFFFFF;stroke:none" d="'
    const p0 = line[0]
    ...
    output += `${line[line.length - 1].x * pageWidth},${pageHeight} `
    output += `${line[0].x * pageWidth},${pageHeight} `
    output += `${p0.x * pageWidth},${p0.y * pageHeight} `
    output += '" />'
  ...
  /*
        if (line.filled) {
        output += `<path style="fill:#FFFFFF;stroke:#000000;stroke-width:${strokeWidth}px;stroke-linecap:butt;stroke-linejoin:miter;stroke-opacity:1" d="`
      } else {
  ...
  element.setAttribute('download', `joydevisualisation_${dateTime}_${format}.svg`)
